            ...
            for (int n = 0; n <= nmax; n++) {
                ...
                if (n < nmax) {
                    for (int m = 0; m < nmax; m++) {
                        E1j1df += -J[j1] * expmth * g(n, m + 1) * ~f[j1][m + 1] * f[j1][m]
                                * f[i][n + 1];
                        E1j2df += -J[i] * expth * g(n, m + 1) * ~f[j2][m + 1] * f[j2][m]
                                * f[i][n + 1];
                    }
                }
                if (n > 0) {
                    for (int m = 1; m <= nmax; m++) {
                        E1j1df += -J[j1] * expth * g(n - 1, m) * ~f[j1][m - 1] * f[j1][m]
                                * f[i][n - 1];
                        E1j2df += -J[i] * expmth * g(n - 1, m) * ~f[j2][m - 1] * f[j2][m]
                                * f[i][n - 1];
                    }
                }
...
double Ecfunc2(unsigned ndim, const double *x, double *grad, void *data) {
    parameters* parms = static_cast<parameters*> (data);
    vector<double> U = parms->U;
    double mu = parms->mu;
    vector<double> J = parms->J;
