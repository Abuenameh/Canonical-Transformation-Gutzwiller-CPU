...
double Encnnfunc(unsigned ndim, const double *x, double *grad, void *data) {
    ...
    for (int i = 0; i < L; i++) {
        ...
        for (int n = 0; n <= nmax; n++) {
            ...
            Ec += (0.5 * U[i] * n * (n - 1) - mu * n + nu[i] * n) * ~f[i][n] * f[i][n];

            if (n < nmax) {
                for (int m = 1; m <= nmax; m++) {
                    Ec += -J[j1] * /*costh*/expth * g(n, m) * ~f[i][n + 1] * ~f[j1][m - 1]
                            * f[i][n] * f[j1][m];
                    Ec += -J[i] * /*costh*/expmth * g(n, m) * ~f[i][n + 1] * ~f[j2][m - 1]
                            * f[i][n] * f[j2][m];
                }
            }
    ...
    if (grad) {
        for (int i = 0; i < L; i++) {
            ...
            doublecomplex Edf = 0;
                ...
                Edf += (0.5 * U[i] * n * (n - 1) - mu * n + nu[i] * n) * f[i][n];

                if (n < nmax) {
                    for (int m = 0; m < nmax; m++) {
                        Edf += -J[j1] * /*costh*/expmth * g(n, m + 1) * ~f[j1][m + 1] * f[j1][m]
                                * f[i][n + 1];
                        Edf += -J[i] * /*costh*/expth * g(n, m + 1) * ~f[j2][m + 1] * f[j2][m]
                                * f[i][n + 1];
                    }
                }
                if (n > 0) {
                    for (int m = 1; m <= nmax; m++) {
                        Edf += -J[j1] * /*costh*/expth * g(n - 1, m) * ~f[j1][m - 1] * f[j1][m]
                                * f[i][n - 1];
                        Edf += -J[i] * /*costh*/expmth * g(n - 1, m) * ~f[j2][m - 1] * f[j2][m]
                                * f[i][n - 1];
                    }
                }
...
double Encfunc(unsigned ndim, const double *x, double *grad, void *data) {
    ...
    for (int i = 0; i < L; i++) {
        ...
        for (int n = 0; n <= nmax; n++) {
            ...
            if (n < nmax) {
                for (int m = 1; m <= nmax; m++) {
                    E1j1 += -J[j1] * /*costh*/expth * g(n, m) * ~f[i][n + 1] * ~f[j1][m - 1]
                            * f[i][n] * f[j1][m];
                    E1j2 += -J[i] * /*costh*/expmth * g(n, m) * ~f[i][n + 1] * ~f[j2][m - 1]
                            * f[i][n] * f[j2][m];
                }
            }
    ...
    if (grad) {
        for (int i = 0; i < L; i++) {
            ...
            for (int n = 0; n <= nmax; n++) {
                ...
                if (n < nmax) {
                    for (int m = 0; m < nmax; m++) {
                        E1j1df += -J[j1] * /*costh*/expmth * g(n, m + 1) * ~f[j1][m + 1] * f[j1][m]
                                * f[i][n + 1];
                        E1j2df += -J[i] * /*costh*/expth * g(n, m + 1) * ~f[j2][m + 1] * f[j2][m]
                                * f[i][n + 1];
                    }
                }
                if (n > 0) {
                    for (int m = 1; m <= nmax; m++) {
                        E1j1df += -J[j1] * /*costh*/expth * g(n - 1, m) * ~f[j1][m - 1] * f[j1][m]
                                * f[i][n - 1];
                        E1j2df += -J[i] * /*costh*/expmth * g(n - 1, m) * ~f[j2][m - 1] * f[j2][m]
                                * f[i][n - 1];
                    }
                }
