...
//	for (int i = 0; i < L; i++) {
	...
	if (n < nmax) {
		Ec = Ec
			+ -J[j1] * costh * g(n, n + 1) * ~f[i][n + 1] * ~f[j1][n] * f[i][n]
				* f[j1][n + 1];
		Ec = Ec
			+ -J[i] * costh * g(n, n + 1) * ~f[i][n + 1] * ~f[j2][n] * f[i][n]
				* f[j2][n + 1];
		...
		if (n > 0) {
			...
			Ec =
				Ec
					+ 0.5 * J[i] * J[i] * cos2th * g(n, n) * g(n - 1, n + 1)
						* ~f[i][n + 1] * ~f[j2][n - 1] * f[i][n - 1]
						* f[j2][n + 1]
						* (1 / eps(U, i, j2, n, n)
							- 1 / eps(U, i, j2, n - 1, n + 1));
		...
		for (int m = 1; m <= nmax; m++) {
			if (n != m - 1 && n < nmax) {
				Ec = Ec
					+ 0.5 * (J[j1] * J[i] * cos2th / eps(U, i, j1, n, m)) * g(n, m)
						* g(m - 1, n + 1) * ~f[i][n + 1] * ~f[j1][m - 1]
						* ~f[j2][m] * f[i][n + 1] * f[j1][m] * f[j2][m - 1];
				Ec = Ec
					+ 0.5 * (J[i] * J[j1] * cos2th / eps(U, i, j2, n, m)) * g(n, m)
						* g(m - 1, n + 1) * ~f[i][n + 1] * ~f[j2][m - 1]
						* ~f[j1][m] * f[i][n + 1] * f[j2][m] * f[j1][m - 1];
				Ec = Ec
					+ 0.5 * (J[j1] * J[k1] * cos2th / eps(U, i, j1, n, m)) * g(n, m)
						* g(m - 1, n + 1) * ~f[i][n + 1] * ~f[j1][m - 1]
						* ~f[k1][n] * f[i][n] * f[j1][m - 1] * f[k1][n + 1];
				Ec = Ec
					+ 0.5 * (J[i] * J[j2] * cos2th / eps(U, i, j2, n, m)) * g(n, m)
						* g(m - 1, n + 1) * ~f[i][n + 1] * ~f[j2][m - 1]
						* ~f[k2][n] * f[i][n] * f[j2][m - 1] * f[k2][n + 1];
				Ec = Ec
					- 0.5 * (J[j1] * J[i] * cos2th / eps(U, i, j1, n, m)) * g(n, m)
